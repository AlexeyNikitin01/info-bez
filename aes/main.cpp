...
void RotBytes(unsigned char *temp) {
...
void SubBytesKey(unsigned char *temp) {
  int i;
  for (i = 0; i < 4; i++) {
...
void NewRc(unsigned char *rc, int n) {
    unsigned int i;
    unsigned char c = 1;
    for (i = 0; i < n - 1; i++) {
        c = ((c << 1) ^ (((c >> 7) & 1) * 0x1b));
...
void Rc(unsigned char *temp, unsigned char *rc, unsigned char *toTemp) {
  int i;
  for (i = 0; i < 4; i++) {
    toTemp[i] = temp[i] ^ rc[i];
  }
}

// 16 * 11; 1 round for copy key
void KeyExpansion(const unsigned char k[16], unsigned char w[176]) {
    for (int i=0; i<16; i++) {
        w[i] = k[i];
    }
...
void DecryptECB(const unsigned char in[], const unsigned char key[]) {
...
int main() {
  unsigned char plain[16] = {'a', 'b', 'c', 'd', 'b', 'c', 'd', 'b', 'c', 'd', 'b', 'c', 'd', 'b', 'c', 'd'};  
  ...
  for (unsigned int i = 0; i < 16; i += 16) {
    EncryptBlock(plain + i, out + i, roundKeys);
  }
